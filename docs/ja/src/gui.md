...
**バッチ選択** — 各行のチェックボックスで複数のエントリを選択できます（将来のバッチ操作用）。
...
| `/` | ファイルツリーの検索バーにフォーカス |
| `Tab` | ファイルツリー ↔ インスペクター間のフォーカス切り替え |
| `Escape` | エントリ選択を解除 |
...
ツールバーの **レポート出力** ボタンで Markdown レポートを出力できます。

JSON / HTML / SARIF 形式は `aaai report` CLI コマンドで出力できます。

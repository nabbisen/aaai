...
impl App {
    ...
    fn before_path_changed(&mut self, s: String) {
        self.before_path = s;
        self.validate_opening();
    }

    fn after_path_changed(&mut self, s: String) {
        self.after_path = s;
        self.validate_opening();
    }

    fn definition_path_changed(&mut self, s: String) {
        self.definition_path = s;
    }

    #[allow(clippy::needless_return, clippy::collapsible_if)]
    fn start_audit(&mut self) -> Task<Message> {
    ...
    fn reason_changed(&mut self, s: String) {
    ...
    fn reason_action(&mut self, action: iced::widget::text_editor::Action) {
    ...
    fn note_changed(&mut self, s: String) {
        self.inspector.note = s;
    }

    fn strategy_selected(&mut self, kind: StrategyKind) {
    ...
    fn checksum_changed(&mut self, s: String) {
    ...
    fn regex_pattern_changed(&mut self, s: String) {
    ...
    fn regex_target_changed(&mut self, new_target: RegexTarget) {
    ...
    fn edit_rule(&mut self, idx: usize) {
    ...
    fn remove_line_rule(&mut self, i: usize) {
        if let AuditStrategy::LineMatch { rules } = &mut self.inspector.strategy {
            if i < rules.len() {
                rules.remove(i);
            }
    ...
    #[allow(clippy::collapsible_if)]
    fn line_rule_action_changed(&mut self, i: usize, new_action: LineAction) {
    ...
    #[allow(clippy::collapsible_if)]
    fn line_rule_line_changed(&mut self, i: usize, s: String) {
        if let AuditStrategy::LineMatch { rules } = &mut self.inspector.strategy {
            if let Some(r) = rules.get_mut(i) {
                r.line = s;
            }
    ...
    fn exact_content_changed(&mut self, s: String) {
    ...
    fn approve_and_save(&mut self) -> Task<Message> {
    ...
    #[allow(clippy::collapsible_if)]
    fn approve_entry(&mut self) -> Task<Message> {
    ...
    fn toggle_batch_select(&mut self, idx: usize) {
    ...
    fn batch_reason_changed(&mut self, s: String) {
        self.batch.shared_reason = s;
    }

    fn batch_strategy_selected(&mut self, kind: StrategyKind) {
        // RFC 035 — payload is already `StrategyKind`.
        self.batch.shared_strategy = kind.to_default_strategy();
    }

    fn open_batch_sheet(&mut self) {
        self.batch_sheet_open = true;
    }

    fn close_batch_sheet(&mut self) {
        self.batch_sheet_open = false;
    }

    #[allow(clippy::needless_return)]
    fn commit_batch_approve(&mut self) -> Task<Message> {
    ...
    #[allow(clippy::needless_return)]
    fn rerun_audit(&mut self) -> Task<Message> {
        // RFC 037 — converted to async; toast fires from RerunDiffReady.
        return self.start_async_rerun();
    }

    fn save_definition(&mut self) -> Task<Message> {
    ...
    #[allow(clippy::needless_return)]
    fn export_report(&mut self) -> Task<Message> {
    ...
    fn definition_save_path_cancelled(&mut self) {
        // User cancelled the dialog — clear any pending-leave flag, no toast.
        self.pending_leave_to_opening = false;
    }

    fn definition_save_path_picked(&mut self, chosen: std::path::PathBuf) {
    ...
    fn report_path_cancelled(&mut self) {
        /* user cancelled */
    }

    fn report_path_picked(&mut self, out: std::path::PathBuf) {
        if let Some(result) = &self.audit_result {
            let before = PathBuf::from(&self.before_path);
            let after = PathBuf::from(&self.after_path);
    ...
    fn search_query_changed(&mut self, s: String) {
        self.search_query = s;
    }

    fn toggle_dir(&mut self, dir: String) {
    ...
    fn diff_loading(&mut self, msg: String) {
    ...
    fn diff_failed(&mut self, err: String) {
        self.is_loading = false;
        self.load_progress = None;
        self.open_error = Some(crate::error::UserError::new(
    ...
    #[allow(clippy::collapsible_if)]
    fn undo_approval(&mut self) -> Task<Message> {
    ...
    fn select_prev(&mut self) -> Task<Message> {
        if let Some(result) = &self.audit_result {
            let visible: Vec<usize> = result
                .results
                .iter()
    ...
    fn set_theme(&mut self, t: AppTheme) {
        self.theme = t;
    ...
    fn pane_resized(&mut self, e: pane_grid::ResizeEvent) {
        self.panes.resize(e.split, e.ratio);
    }

    fn pane_focused(&mut self, p: pane_grid::Pane) {
        self.focus = Some(p);
    }

    fn noop(&mut self) {}

    fn deselect_entry(&mut self) {
        self.selected_index = None;
    }

    fn set_diff_view_mode(&mut self, mode: DiffViewMode) {
        self.diff_view_mode = mode;
    }

    #[allow(clippy::needless_return)]
    fn pick_before_folder(&mut self) -> Task<Message> {
    ...
    #[allow(clippy::needless_return)]
    fn pick_after_folder(&mut self) -> Task<Message> {
    ...
    #[allow(clippy::needless_return)]
    fn pick_definition_file(&mut self) -> Task<Message> {
    ...
    #[allow(clippy::needless_return)]
    fn pick_ignore_file(&mut self) -> Task<Message> {
    ...
    fn before_folder_picked(&mut self, opt: Option<std::path::PathBuf>) {
    ...
    fn after_folder_picked(&mut self, opt: Option<std::path::PathBuf>) {
        if let Some(path) = opt {
    ...
    fn definition_file_picked(&mut self, opt: Option<std::path::PathBuf>) {
        if let Some(path) = opt {
    ...
    fn ignore_file_picked(&mut self, opt: Option<std::path::PathBuf>) {
        if let Some(path) = opt {
    ...
    fn toggle_optional_settings(&mut self) {
        self.optional_settings_expanded = !self.optional_settings_expanded;
    }

    fn file_hover_enter(&mut self) {
    ...
    fn file_hover_leave(&mut self) {
        self.file_hovering = false;
    }

    fn file_dropped(&mut self, path: std::path::PathBuf) -> Task<Message> {
        self.file_hovering = false;
    ...
    fn focus_inspector_reason(&mut self) {
        // RFC 005: update logical focus; inspector reason input highlighted
        self.focus_target = FocusTarget::Inspector;
    }

    fn ticket_changed(&mut self, s: String) {
        self.inspector.ticket = s;
    }

    fn approved_by_changed(&mut self, s: String) {
        self.inspector.approved_by = s;
    }

    fn expires_at_changed(&mut self, s: String) {
        self.inspector.expires_at_str = s;
    }

    fn apply_template(&mut self, id: String) {
    ...
    fn ignore_path_changed(&mut self, s: String) {
        self.ignore_path = s;
    }

    fn profile_name_changed(&mut self, s: String) {
    ...
    fn switch_locale(&mut self, code: String) {
        rust_i18n::set_locale(&code);
        self.locale = code;
    }

    fn rerun_diff_ready(&mut self, result: Result<Vec<aaai::DiffEntry>, String>) {
        self.is_loading = false;
        self.load_progress = None;
        match result {
            Ok(fresh_diffs) => {
    ...
    fn nav_guard_cancel(&mut self) {
        self.nav_guard_open = false;
        self.nav_guard_show_discard = false;
    }

    fn nav_guard_reveal_discard(&mut self) {
        self.nav_guard_show_discard = true;
    }

    fn nav_guard_discard_and_leave(&mut self) {
        self.nav_guard_open = false;
        self.dirty = false;
        self.do_leave_to_opening();
    }

    fn nav_guard_save_and_leave(&mut self) -> Task<Message> {
    ...
    fn pattern_changed(&mut self, s: String) {
        self.inspector.pattern_path = s;
        self.validate_pattern();
    }

    fn apply_pattern_suggestion(&mut self, s: String) {
        self.inspector.pattern_path = s;
        self.validate_pattern();
    }

    #[allow(clippy::needless_return)]
    fn diff_before_scrolled(&mut self, vp: iced::widget::scrollable::Viewport) -> Task<Message> {
    ...
    #[allow(clippy::needless_return)]
    fn diff_after_scrolled(&mut self, vp: iced::widget::scrollable::Viewport) -> Task<Message> {
        if self.diff_scroll_syncing {
            self.diff_scroll_syncing = false;
            return Task::none();
        }
    ...
    #[allow(clippy::collapsible_if)]
    fn revert_selected_entry(&mut self) -> Task<Message> {

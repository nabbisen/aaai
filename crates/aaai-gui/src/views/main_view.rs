...
pub fn view(app: &App) -> Element<'_, Message> {
    ...
    column![
        toolbar, filter_bar, pg, bottom_bar, // RFC 008: fixed bottom action bar
    ]
    .spacing(0)
    .into()
}
...
fn build_toolbar<'a>(app: &'a App) -> Element<'a, Message> {
    ...
    let run_btn = toolbar_btn(
        "▶",
        t!("toolbar.run_audit").to_string(),
        Message::RerunAudit,
    );
    let report_btn = toolbar_btn(
        "↑",
        t!("toolbar.report_output").to_string(),
        Message::ExportReport,
    );
    let undo_btn = toolbar_btn("↩", t!("toolbar.undo").to_string(), Message::UndoApproval);

    let save_mark_text = app
        .last_saved_at
        .map(|t| format!("✓ {}", crate::util::humanize_since(t)));
    let report_mark_text = app
        .last_reported_at
        .map(|t| format!("✓ {}", crate::util::humanize_since(t)));
    ...
    let status_element: Element<'_, Message> = if app.audit_dirty && app.is_loading {
        text(format!("○ {}", t!("toolbar.rerunning")))
            .size(app.design_tokens.typography.label.size)
            .line_height(app.design_tokens.typography.label.line_height)
            .color(crate::theme::status_color(
                aaai::AuditStatus::Pending,
                &app.design_tokens,
                app.theme.is_high_contrast(),
            ))
            .into()
...
fn build_filter_bar<'a>(app: &'a App) -> Element<'a, Message> {
    ...
    let make_btn = |base_key: &'static str, mode: FilterMode, count: Option<usize>| {
        ...
        if active {
            ...
            container(btn).style(move |_| iced::widget::container::Style {
                background: Some(iced::Background::Color(Color { a: 0.18, ..accent })),
                border: iced::Border {
                    radius: 4.0.into(),
                    ..Default::default()
                },
    ...
    container(column![filter_row, legend_popup].spacing(0))
        .width(Length::Fill)
        .style(panel_style(app.design_tokens.clone()))
        .into()
}
...
fn build_file_tree<'a>(app: &'a App) -> Element<'a, Message> {
    ...
    let coach_line: Option<Element<'_, Message>> = if !app.coach_dismissed {
        ...
        Some(
            ...
            .style({
                let accent = crate::style::to_iced(app.design_tokens.palette.accent);
                move |_| iced::widget::container::Style {
                    background: Some(iced::Background::Color(Color { a: 0.10, ..accent })),
                    ..Default::default()
                }
            })
            .into(),
    ...
    for (idx, far) in result.results.iter().enumerate() {
        ...
        if !q.is_empty() && !far.diff.path.to_lowercase().contains(&q) {
            continue;
        }
        ...
        if !dir.is_empty() && app.collapsed_dirs.contains(&dir) {
            continue;
        }
    ...
    let tree_scroll = scrollable(column(items).spacing(0).width(Length::Fill))
        .width(Length::Fill)
        .height(Length::Fill);
...
fn build_file_row<'a>(
    ...
    let warn_badge: Option<Element<'_, Message>> = if !far.warnings.is_empty() {
        Some(
            ...
            .style({
                ...
                move |_| iced::widget::container::Style {
                    background: Some(iced::Background::Color(Color { a: 0.25, ..warning })),
                    border: iced::Border {
    ...
    let full_row = row![name_row, space().width(Length::Fill), dtype_tag,]
        .spacing(app.design_tokens.spacing.xs)
        .align_y(iced::Alignment::Center);
...
fn status_icon(
    ...
    let (sym, color) = match status {
        AuditStatus::Ok => (
            "✓",
            crate::theme::status_color(AuditStatus::Ok, tokens, is_hc),
        ),
        AuditStatus::Pending => (
            "⚠",
            crate::theme::status_color(AuditStatus::Pending, tokens, is_hc),
        ),
        AuditStatus::Failed => (
            "✗",
            crate::theme::status_color(AuditStatus::Failed, tokens, is_hc),
        ),
        AuditStatus::Error => (
            "!",
            crate::theme::status_color(AuditStatus::Error, tokens, is_hc),
        ),
        AuditStatus::Ignored => (
            "—",
            crate::theme::status_color(AuditStatus::Ignored, tokens, is_hc),
        ),
    };
    text(sym)
...
fn build_bottom_bar<'a>(app: &'a App) -> Element<'a, Message> {
    ...
    let can_approve = app.selected_index.is_some() && app.inspector.validation.can_approve();
    ...
    .on_press_maybe(if can_approve {
        Some(Message::ApproveAndSave)
    } else {
        None
    })
    .padding(Padding::from([
        app.design_tokens.spacing.md,
        app.design_tokens.spacing.xl,
    ])) // ABDD ≥44px
    ...
    let count_label: Element<'_, Message> =
        if let Some(s) = app.audit_result.as_ref().map(|r| &r.summary) {
...
fn empty_state_inspector<'a>(tokens: snora::design::Tokens) -> Element<'a, Message> {
    ...
    let body = column![
        ...
        text(format!(
            "←  {}",
            t!("empty_state.inspector_no_selection_hint")
        ))
        .size(tokens.typography.body_small.size)
        .line_height(tokens.typography.body_small.line_height)
        .color(crate::style::to_iced(tokens.palette.text_muted)),

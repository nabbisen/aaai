...
use aaai::{
    AuditStatus, DiffType, FileAuditResult,
...
pub fn view<'a>(app: &'a App, far: &'a FileAuditResult) -> Element<'a, Message> {
    ...
    // Expiry badge
    let expiry_badge: Option<Element<'_, Message>> = far.entry.as_ref().and_then(|e| {
        if e.is_expired() {
            Some(colored_badge(
                t!("expiry.expired_badge").to_string(),
                crate::style::to_iced(app.design_tokens.palette.danger),
                &app.design_tokens,
            ))
        } else if e.expires_soon(30) {
            Some(colored_badge(
                t!("expiry.soon_badge").to_string(),
                crate::style::to_iced(app.design_tokens.palette.warning),
                &app.design_tokens,
            ))
        } else {
            None
    ...
    let pattern_section: Option<iced::Element<'_, Message>> = if ins.use_pattern {
        let pattern_input = text_input(&t!("inspector.pattern_placeholder"), &ins.pattern_path)
        ...
        let pat_status: iced::Element<'_, Message> = match &ins.validation.pattern_error {
            Some(err) => text(format!("✗ {}", err))
                ...
                .into(),
            None if !ins.pattern_path.trim().is_empty() => text("✓")
                .size(app.design_tokens.typography.body_small.size)
                .line_height(app.design_tokens.typography.body_small.line_height)
        ...
        let suggestions_row: Option<iced::Element<'_, Message>> =
            if !ins.pattern_suggestions.is_empty() {
                let label = text(t!("inspector.pattern_suggestions").to_string())
                    ...
                    .line_height(app.design_tokens.typography.body_small.line_height)
                    .color(crate::style::to_iced(app.design_tokens.palette.text_muted));
                let chips: Vec<iced::Element<'_, Message>> = ins
                    .pattern_suggestions
                    .iter()
                    .map(|s| {
                        let s2 = s.clone();
        ...
        Some(
            column(
                [
                    Some(
                        semibold_text(
                            t!("inspector.pattern_label").to_string(),
                            &app.design_tokens,
                        )
                        .into(),
                    ),
    ...
    let approved_by_label = semibold_text(
        t!("inspector.approved_by_label").to_string(),
        &app.design_tokens,
    );
    ...
    let expires_label = semibold_text(
        t!("inspector.expires_at_label").to_string(),
        &app.design_tokens,
    );
    ...
    // ── Section: strategy ────────────────────────────────────────────
    let strategy_label = semibold_text(
        t!("inspector.strategy_label").to_string(),
        &app.design_tokens,
    );
    ...
    .collect();
    let strategy_selected = strategy_options
        .iter()
    ...
    .padding(app.design_tokens.spacing.xs)
    .text_size(11);
    ...
    let warning_section: Option<Element<'a, Message>> = if !far.warnings.is_empty() {
        let warn_items: Vec<Element<'a, Message>> = far
            .warnings
            .iter()
            .map(|w| {
            ...
            container(column(warn_items).spacing(app.design_tokens.spacing.xs))
                ...
                .style({
                    ...
                    move |_| iced::widget::container::Style {
                        background: Some(iced::Background::Color(Color { a: 0.12, ..warning })),
                        border: iced::Border {
    ...
    let mut children: Vec<Element<'_, Message>> = vec![header_row.into(), divider.into()];
    ...
    children.extend([reason_label.into(), reason_input.into()]);
    ...
    children.extend([
        iced::widget::rule::horizontal(1).into(),
        strategy_label.into(),
        row![strategy_pick, space().width(Length::Fill)]
            .spacing(app.design_tokens.spacing.xs)
            .into(),
        strategy_desc.into(),
        strategy_form,
    ...
    let effective_advanced_expanded =
        app.advanced_inspector_expanded || ins.validation.expires_at_error.is_some();
    ...
    if effective_advanced_expanded {
        children.extend([
            ...
            ticket_label.into(),
            ticket_input.into(),
            approved_by_label.into(),
            approved_by_input.into(),
            expires_label.into(),
            expires_input.into(),
            iced::widget::rule::horizontal(1).into(),
            tmpl_label.into(),
...
fn build_strategy_form<'a>(
    ins: &'a InspectorState,
    tokens: &'a snora::design::Tokens,
) -> Element<'a, Message> {
    match &ins.strategy {
        AuditStrategy::None => text(t!("inspector.no_content_inspection").to_string())
        ...
        AuditStrategy::LineMatch { rules } => {
            ...
            for (i, rule) in rules.iter().enumerate() {
                ...
                if is_editing {
                    ...
                    let action_selected = action_options
                        .iter()
                    ...
                    let line_input = text_input(
                        &t!("inspector.linematch_line_placeholder").to_string(),
                        &rule.line,
                    )
                    ...
                    let (bg, label_color, action_label) = if rule.action == LineAction::Removed {
                        ...
                        (Color { a: 0.10, ..danger }, danger, "Removed")
                    } else {
                        let success = crate::style::to_iced(tokens.palette.success);
                        (Color { a: 0.10, ..success }, success, "Added")
                    };
                    let block = button(
                        container(
                            column![
        ...
        AuditStrategy::Regex { pattern, target } => {
            ...
            column![
                text(t!("inspector.regex_pattern_label").to_string())
                    ...
                    .line_height(tokens.typography.label.line_height),
                text_input(
                    &t!("inspector.regex_pattern_placeholder").to_string(),
                    pattern
                )
                ...
                pick_list(
                    target_options,
                    target_selected,
                    |o: crate::util::LocalizedOption<RegexTarget>| Message::RegexTargetChanged(
                        o.value
                    )
                )
                .padding(tokens.spacing.xs),
        ...
        AuditStrategy::Exact { expected_content } => column![
            text(t!("inspector.exact_label").to_string())
                .size(tokens.typography.label.size)
                .line_height(tokens.typography.label.line_height),
            text_input(
                &t!("inspector.exact_content_placeholder").to_string(),
                expected_content
            )

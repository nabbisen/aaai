use super::*;
use crate::config::definition::{AuditEntry, AuditStrategy, LineAction, LineRule};
...
fn large_file_with_linematch_warns() {
    ...
    assert!(
        warns
            .iter()
            .any(|w| matches!(w, AuditWarning::LargeFileStrategy { .. })),
...
fn large_file_with_checksum_no_warn() {
    ...
    let entry = make_entry(AuditStrategy::Checksum {
...
fn none_strategy_on_modified_warns() {
    ...
    assert!(
        warns
            .iter()
            .any(|w| matches!(w, AuditWarning::NoStrategyOnModified { .. }))
    );
}
...
fn no_approver_warns() {
    ...
    assert!(
        warns
            .iter()
            .any(|w| matches!(w, AuditWarning::NoApprover { .. }))
    );
}

...
pub fn build_html(
    ...
    for r in &result.results {
        if r.diff.diff_type == crate::diff::entry::DiffType::Unchanged {
            continue;
        }
        ...
        // F2 — every sibling interpolation in this file (`reason` here,
        // `path`, both roots, and the definition path below) is already
        // wrapped in `html_escape`; this was the one that was missed.
        let ticket = r
            .entry
            .as_ref()
            ...
            .map(|t| {
                format!(
                    ...
                    html_escape(&masking.mask(t))
                )
            })
            .unwrap_or_default();
        let binary_badge = if r.diff.is_binary {
...
<p style="margin-top:20px;font-size:0.75rem;color:#aaa">Generated by aaai vVERSION_PLACEHOLDER</p>
</body></html>"#,
        verdict_color = verdict_color,
        before = html_escape(&masking.mask(&before_root.display().to_string())),
        after = html_escape(&masking.mask(&after_root.display().to_string())),
        def_line = definition_path
            .map(|p| format!(
                "&nbsp;·&nbsp; Definition: <code>{}</code>",
                html_escape(&masking.mask(&p.display().to_string()))
            ))
            .unwrap_or_default(),
        ok = s.ok,
        pending = s.pending,
    ...
    let html_base = html_base.replace("vVERSION_PLACEHOLDER", &format!("v{version}"));
